...
def run(cfg,out):
    torch.set_num_threads(2); device=cfg['device'];out=Path(out);out.mkdir(parents=True,exist_ok=True)
    ...
    for seed in cfg['seeds']:
        ...
        for step in range(cfg['updates']):
            ...
            loss=sum(F.cross_entropy(pred['logits'][k],batch['targets'][k]) for k in FIELDS)
            opt.zero_grad();loss.backward();torch.nn.utils.clip_grad_norm_(model.parameters(),1.);opt.step()
            if step%100==0:curve.append(dict(step=step,loss=float(loss.detach())))
