"""Coordinator-released P01 subprocess with external occupancy accounting."""
import argparse
import datetime
import hashlib
import json
...
def main():
    ...
    if args.cap > json.loads(config.read_text())['requested_process_cap_seconds']:
    ...
    command = [args.python, '-m', 'topoformer.campaign_composition_acquire', '--config', str(config), '--output', args.output, '--device', 'cuda']
    ...
    record = dict(started_utc=started, ended_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                  ...
                  command=command, config_sha256=hashlib.sha256(config.read_bytes()).hexdigest(),
