"""Coordinator-released C02 arm subprocess with external occupancy accounting."""
import argparse
import datetime
import hashlib
import json
...
def main():
    ...
    parser.add_argument('--arm', choices=('static','rekey'), required=True)
    args = parser.parse_args()
    if args.cap <= 0: raise ValueError('positive process cap required')
    config = Path(args.config)
    if args.cap > json.loads(config.read_text())['requested_arm_caps_seconds'][args.arm]:
    ...
    command = [args.python, '-m', 'topoformer.campaign_composition_rekey_study', '--config', str(config), '--output', args.output, '--device', 'cuda', '--arm', args.arm]
    ...
    record = dict(started_utc=started, ended_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                  ...
                  command=command, arm=args.arm, config_sha256=hashlib.sha256(config.read_bytes()).hexdigest(),
