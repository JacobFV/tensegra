...
def update(head, optimizer, generator, z, y, steps, batch_size, callback=None):
    visited=torch.zeros(len(z),dtype=torch.bool,device=z.device);losses=[]
...
def run(cfg,out):
    ...
    for field in ('checkpoint','reference_path','reference_manifest'):
    ...
    replay_losses,visited=update(head,optimizer,generator,z,y,cfg['replay_steps'],cfg['batch_size'])
    ...
    endpoint={'state':cpu_tree(head.state_dict()),'optimizer':cpu_tree(optimizer.state_dict()),'rng_state':generator.get_state().cpu(),
              'mean':mean.cpu(),'scale':scale.cpu(),'step':cfg['replay_steps']}
    ...
    for arm,lr in cfg['forks'].items():
        ...
        losses,seen=update(h,opt,rng,z,y,cfg['extra_steps'],cfg['batch_size'],observe)
        ...
        torch.save(dict(state=cpu_tree(h.state_dict()),optimizer=cpu_tree(opt.state_dict()),rng_state=rng.get_state().cpu(),
                        mean=mean.cpu(),scale=scale.cpu(),step=cfg['replay_steps']+cfg['extra_steps']),checkpoint)
    ...
    manifest=dict(config=cfg,config_sha256=hashlib.sha256(json.dumps(cfg,sort_keys=True).encode()).hexdigest(),
        ...
        replay=dict(exact_parameters=True,exact_normalization=True,exact_loss_sequence=True,visited_row_bits_little_endian=bits,
