"""Coordinator-released C03 arm subprocess with external occupancy accounting."""
import argparse
import datetime
import hashlib
import json
...
def main():
    ...
    parser.add_argument('--arm', choices=('static','roles'), required=True)
    ...
    command = [args.python, '-m', 'topoformer.campaign_composition_role_study', '--config', str(config), '--output', args.output, '--device', 'cuda', '--arm', args.arm]
