...
def run(config):
    ...
    for row in audit['rows']:
        ...
        public,_=base.surface_input(example,'english');gold=base.targets(example.privileged.graph,public,128,vocab,'english')
    ...
    for old in archive['runs']:
        ...
        manifest=dict(config=config,source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),training_archive_sha256=hashlib.sha256(Path(config['training_archive']).read_bytes()).hexdigest(),audit_sha256=hashlib.sha256(Path(config['audit']).read_bytes()).hexdigest(),value_vocabulary=vocab,scope='Frozen inference; thresholds inherited only from TRAIN8, no optimizer or fresh calibration',runs=runs)
