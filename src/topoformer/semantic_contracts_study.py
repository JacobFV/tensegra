...
from .semantic_contracts import SlotScorer,slot_objective,single_slot_labels,train_relation_thresholds
...
def evaluate(model,examples,thresholds=None):
    rows=[]
    with torch.no_grad():
        for nodes,pairs,gold,audit in examples:
            edge,slots=model(nodes,pairs);pred=edge.gt(0 if thresholds is None else thresholds).cpu().reshape_as(gold['edges']);slot=slots.argmax(-1).cpu().reshape_as(gold['slots'])-1
    ...
    for seed in config['seeds']:
      for interaction in (False,True):
       for conditional in (False,True):
        ...
        calibrated=None
        if config.get('calibrate_train',False):
            with torch.no_grad():
                predictions=[model(n,p)[0] for n,p,_,_ in examples]
                truths=[g['edges'].flatten(0,1).to(device) for _,_,g,_ in examples]
                thresholds,records=train_relation_thresholds(torch.cat(predictions),torch.cat(truths))
                calibrated=dict(policy='final TRAIN entries only, minimum classification errors, lowest threshold tie',relations=records,rows=evaluate(model,examples,thresholds))
        if device=='cuda':torch.cuda.synchronize()
        checkpoint=out/f'head-{interaction}-conditional-{conditional}-seed{seed}.pt';torch.save(model.state_dict(),checkpoint)
        runs.append(dict(calibrated=calibrated,initial_state_sha256=initial_hash,final_state_sha256=state_hash(model),checkpoint_sha256=hashlib.sha256(checkpoint.read_bytes()).hexdigest(),optimizer_presentations=config['updates']*len(examples),actual_unique_graphs=len(examples),losses=losses,process_maxrss_kib=resource.getrusage(resource.RUSAGE_SELF).ru_maxrss,seed=seed,head='interaction' if interaction else 'additive',objective='edge_conditional' if conditional else 'all_pairs',parameters=sum(p.numel() for p in model.parameters()),seconds=time.monotonic()-start,peak_cuda_allocated=torch.cuda.max_memory_allocated() if device=='cuda' else None,curve=curve))
