"""A14 fixed confirmation policies; oracle policy remains privileged."""
import math
import torch
...
@torch.no_grad()
def forward(model, batch, records, edge_indices, successor, policy='unchanged', *, instrument=True):
    ...
    for t in reversed(range(batch.relations.shape[1])):
        ...
        edge_weight=F.one_hot(edge_argmax,edge_soft.shape[-1]).to(edge_soft.dtype) if policy=='both_hard' else edge_soft
        ...
        elif policy=='both_hard':weights=F.one_hot(destination_argmax,n).to(destination_soft.dtype)
    ...
    if not instrument:
        return dict(logits=torch.stack(logits,1),routes=torch.stack(routes,1),
                    weights=torch.stack(weights_out,1),edge_mass=torch.stack(masses,1),diagnostics={})
