"""S12: frozen paired continuation; reserved evaluation only at the fixed endpoint."""
...
def run(config):
    if config.get('budget_status')!='frozen':raise ValueError('confirmation budget not frozen')
    if config['seed'] not in (701,702,703) or config['updates']!=24576:raise ValueError('unregistered confirmation recipe')
    ...
    confirmation_path=data/'reserved_confirmation.jsonl.gz'
    if digest(confirmation_path)!=config['confirmation_sha256']:raise ValueError('confirmation cache changed')
    confirmation=Dataset(load_cache(confirmation_path),vocab)
    if len(confirmation)!=1024:raise ValueError('confirmation support changed')
    ...
    parent=json.load(gzip.open(config['parent_manifest'],'rt'))
    if digest(config['parent_manifest'])!=config['parent_manifest_sha256']:raise ValueError('parent manifest changed')
    if parent['config']['seed']!=config['seed'] or parent['config']['updates']!=16384:raise ValueError('wrong parent seed/exposure')
    if start_update!=16384:raise ValueError('wrong parent checkpoint update')
    del checkpoint
    torch.cuda.reset_peak_memory_stats()
    source_names=('campaign_semantics_continue.py','campaign_semantics_lr.py','campaign_semantics_lr_train.py','campaign_semantics_confirmation.py','campaign_semantics_confirmation_train.py','campaign_semantics.py','campaign_semantics_data.py','semantic_curriculum.py','semantic_text_acquisition.py','semantic_scaling.py','semantic_contracts.py','thinking.py','thinking_language.py','semantic_graph.py','tcn_data.py')
    manifest=dict(config=config,config_sha256=hashlib.sha256(json.dumps(config,sort_keys=True).encode()).hexdigest(),source_sha256={n:digest(Path(__file__).with_name(n)) for n in source_names},data_audit_sha256=digest(data/'audit.json'),parameters=sum(p.numel() for p in model.parameters()),width=1024,workspace_rows=8,initial_state_sha256=initial,parent_checkpoint_sha256=config['parent_checkpoint_sha256'],inherited_optimizer_steps=inherited_steps,start_update=start_update,learning_rate_override=lr_override,parent_manifest_sha256=config['parent_manifest_sha256'],confirmation_sha256=config['confirmation_sha256'],curves=curves,losses=losses)
    for update in range(start_update,config['updates']+1):
        if update in config['checkpoints']:
            evaluation_set=dev if update==start_update else confirmation
            record=calibrated_evaluation(model,train,evaluation_set,out,update,config['calibration_count'],len(evaluation_set))
            record['evaluation_split']='development_replay' if update==start_update else 'reserved_confirmation'
