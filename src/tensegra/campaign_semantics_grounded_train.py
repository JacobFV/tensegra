"""S09 matched-data acquisition for a predicted-copy-conditioned context read."""
...
def run(config):
    if not config.get('mechanical_profile') and config.get('budget_status')!='frozen':raise ValueError('main budget not frozen after measured profile')
    ...
    model=CopyConditionedActor(context_read=config['context_read'],read_scale=config['read_scale'],value_count=len(vocab),width=1024,capacity=128,workspace_rows=8,microsteps=2,autocast_dtype='bfloat16').to(config['device'])
    ...
    source_names=('campaign_semantics_grounded_actor.py','campaign_semantics_grounded_train.py','campaign_semantics.py','campaign_semantics_data.py','semantic_curriculum.py','semantic_text_acquisition.py','semantic_scaling.py','semantic_contracts.py','thinking.py','thinking_language.py','semantic_graph.py','tcn_data.py')
    manifest=dict(config=config,config_sha256=hashlib.sha256(json.dumps(config,sort_keys=True).encode()).hexdigest(),source_sha256={n:digest(Path(__file__).with_name(n)) for n in source_names},data_audit_sha256=digest(data/'audit.json'),parameters=sum(p.numel() for p in model.parameters()),active_parameters=sum(p.numel() for p in model.parameters() if p.requires_grad),width=1024,workspace_rows=8,context_token_lengths=sorted({r['tokens'] for r in train.rows}),context_read=config['context_read'],read_scale=config['read_scale'],initial_state_sha256=initial,curves=curves,losses=losses)
