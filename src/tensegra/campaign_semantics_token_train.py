"""S06 matched-data acquisition for the contextual token-state intervention."""
...
from .campaign_semantics_token_actor import ContextualTokenActor
...
def run(config):
    ...
    model=ContextualTokenActor(value_count=len(vocab),width=1024,capacity=128,workspace_rows=8,microsteps=2,autocast_dtype='bfloat16').to(config['device'])
    ...
    source_names=('campaign_semantics_token_actor.py','campaign_semantics_token_train.py','campaign_semantics.py','campaign_semantics_data.py','semantic_curriculum.py','semantic_text_acquisition.py','semantic_scaling.py','semantic_contracts.py','thinking.py','thinking_language.py','semantic_graph.py','tcn_data.py')
    manifest=dict(config=config,config_sha256=hashlib.sha256(json.dumps(config,sort_keys=True).encode()).hexdigest(),source_sha256={n:digest(Path(__file__).with_name(n)) for n in source_names},data_audit_sha256=digest(data/'audit.json'),parameters=sum(p.numel() for p in model.parameters()),active_parameters=sum(p.numel() for p in model.parameters() if p.requires_grad),width=1024,workspace_rows='public_token_count',state_token_lengths=sorted({r['tokens'] for r in train.rows}),allocated_inactive_initial_rows=8,initial_state_sha256=initial,curves=curves,losses=losses)
