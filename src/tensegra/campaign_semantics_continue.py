"""S03 unchanged actor/objective with optimizer-preserving exposure continuation."""
...
def assert_replay(old_path,new_path):
    old=json.load(gzip.open(old_path,'rt'));new=json.load(gzip.open(new_path,'rt'))
    for group in ('train_rows','rows'):
        if len(old[group])!=len(new[group]):raise ValueError('replay row count mismatch')
        for a,b in zip(old[group],new[group]):
            for field in ('seed','raw','calibrated_edges','target'):
                if a[field]!=b[field]:raise ValueError(f'replay mismatch: {group}/{field}')
    if old['thresholds']!=new['thresholds']:raise ValueError('replay thresholds differ')
    return True


def next_indices(order,position,schedule,batch_size):
    indices=[]
    ...
    return indices,order,position


def first_batch_probe(checkpoint,train,config):
    schedule=torch.Generator();schedule.set_state(checkpoint['schedule']);generator=torch.Generator();generator.set_state(checkpoint['generator'])
    indices,_,_=next_indices(checkpoint['order'],checkpoint['position'],schedule,config['batch_size'])
    pairs=[sampled_pairs(train[i][1],generator,config['negative_pairs']) for i in indices]
    return dict(indices=indices,seeds=[train.rows[i]['seed'] for i in indices],pair_sha256=[hashlib.sha256(p.numpy().tobytes()).hexdigest() for p in pairs])
...
def run(config):
    ...
    source_names=('campaign_semantics_continue.py','campaign_semantics.py','campaign_semantics_data.py','semantic_curriculum.py','semantic_text_acquisition.py','semantic_scaling.py','semantic_contracts.py','thinking.py','thinking_language.py','semantic_graph.py','tcn_data.py')
    manifest=dict(config=config,config_sha256=hashlib.sha256(json.dumps(config,sort_keys=True).encode()).hexdigest(),source_sha256={n:digest(Path(__file__).with_name(n)) for n in source_names},data_audit_sha256=digest(data/'audit.json'),parameters=sum(p.numel() for p in model.parameters()),width=1024,workspace_rows=8,initial_state_sha256=initial,parent_checkpoint_sha256=config['parent_checkpoint_sha256'],inherited_optimizer_steps=inherited_steps,start_update=start_update,curves=curves,losses=losses)
