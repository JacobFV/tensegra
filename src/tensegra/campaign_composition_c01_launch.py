"""Coordinator-released C01 phase subprocess with external occupancy accounting."""
import argparse
import datetime
import hashlib
import json
...
def main():
    ...
    parser.add_argument('--phase', choices=('hybrid','n1','n1_frozen','n2'), required=True)
    ...
    command = [args.python, '-m', 'topoformer.campaign_composition_study', '--config', str(config), '--output', args.output, '--device', 'cuda', '--phase', args.phase]
