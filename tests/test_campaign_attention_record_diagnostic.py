...
from topoformer.campaign_attention_record_diagnostic import forward,POLICIES
from topoformer.campaign_attention_selector import generate,oracle_successors
...
def test_zero_change_exact_and_all_policies_frozen(primary):
    ...
    for policy in POLICIES:
        ...
        if policy in ['record_hard','both_hard']:assert z['diagnostics']['key_mse_to_record_argmax'].eq(0).all()
        if policy in ['destination_hard','both_hard']:assert z['diagnostics']['payload_mse_to_destination_argmax'].eq(0).all()
...
def test_mechanical_export_roundtrip(tmp_path):
    ...
    from topoformer.campaign_attention_record_diagnostic_study import run
