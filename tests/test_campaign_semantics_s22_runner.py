...
pkg=types.ModuleType('s22guard');pkg.__path__=[str(Path('src/topoformer').resolve())];sys.modules['s22guard']=pkg
spec=importlib.util.spec_from_file_location('s22guard.campaign_semantics_s22_freeze','src/topoformer/campaign_semantics_s22_freeze.py');f=importlib.util.module_from_spec(spec);spec.loader.exec_module(f)
class Guards(unittest.TestCase):
 ...
 def test_prepared_cannot_launch(self):
  with self.assertRaises(ValueError):f.verify(self.config(),Path('src/topoformer'),180)
 def test_main(self):
