...
## Frozen controlled main schedule

The `economy_v1` coverage contract now matches runner `02be34e` / frozen source `c258785`: step zero evaluates minimum and maximum configured depths, intermediate checkpoints evaluate minimum depth, and final evaluates all configured depths. Final OOD conditions are the seven runner-declared surface/motif/composition/word-order/distractor/delay cases when enabled. Final local minimum/maximum anchors additionally require eight frozen interventions plus separate `oracle_trace` and `oracle_minimal` rows. Both oracle conditions are excluded from autonomous aggregates/contrasts. Legacy artifacts without the schedule field keep full core-grid expectations.

Merge validation permits only documented per-shard provenance variation: seed-specific config hash, initial/checkpoint maps, elapsed time and UTC start. All other manifest fields, including source/git/runtime versions, remain identical. It validates resolved config digests, row initialization and checkpoint-state hashes against producer manifests, and checkpoint file digests before copying. The combined manifest unions initialization/checkpoint maps and records summed shard elapsed time explicitly as **not parallel wall time**. Execution backend labels (`learned`, `exact`, `none`) are retained in aggregate cells; numeric semantic agreement under the learned backend must not be described as exact-interpreter invocation success.

Validation now has 13 passing stdlib tests, including exact economy-grid membership/coverage and modern provenance union with checkpoint-corruption rejection. Main compute froze before this artifact-only analyzer revision; its own SHA256 is recorded independently, and frozen source files remain untouched.
