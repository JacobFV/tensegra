...
## Completed adaptive audit

The independent streaming audit passes **6/6 follow-up runs** at frozen source `11b921a18d5f6839b11e49ca6b17db6eb8c83547`, each with 400 steps and all 20 matrix cells. Full initial-state hashes—not only ordinary-parameter hashes—match between fixed and adaptive arms for every seed. Configuration/source, schedules, evaluation data, initialization/final pairing, finite metrics and independently reconstructed trajectory/transition counts all pass. Raw/config/script hashes accompany `results/stage4/adaptive/audit.json`.

Both arms meet the per-seed corner gate with identical D64/N128 task scores **.9921875, 1.0000, .9765625** and all complete paths **1.0000**. Across 60 paired seed × matrix cells, task accuracy is identical in 59; the remaining cell loses one of 128 answers under adaptation. Effective adaptive strengths at the corner are **3.9890456, 3.9889312, 3.9891436**, versus exactly four in the fixed control. The intervention is therefore tiny under already-confident binding, and this experiment demonstrates no performance benefit of confidence modulation. It neither tests uncertainty-triggered recovery nor confidence-driven acquisition of the stable identity register.

Main and adaptive artifacts are approved for reporting, with their distinct sources and objectives preserved. The initialized explicit pointer prior remains central to the strongest result; interpreter integration and broad claims about autonomous learned binding are unsupported by this audit.
