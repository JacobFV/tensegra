# Stage 11: trained horizons and public-text acquisition

Baseline `e10b80a`; standard width **1024**. Independent tracks advance under their own prerequisites. Autonomous composition and supervision withdrawal remain blocked.

- [Design and budget](stage11-design.md)
- [Experiment registry](stage11-registry.json)
- [Frozen belief reference](stage11-belief-reference.md)
- [Independent review](stage11-review.md)
- [Claims and their prerequisites](stage11-claim-map.md)
- [Public-text acquisition protocol](stage11-semantic-text-design.md)
- [Return training protocol](stage11-return-design.md) and [return report](stage11-return-report.md)
- [Semantic acquisition and fresh-transfer report](stage11-semantic-text-report.md)
- [Prospective fresh-construction protocol](stage11-semantic-fresh-design.md)

The preferred belief interface separates observation addresses from semantic matching. New experiments concern return training horizons and public-text graph acquisition; they do not directly test programmable attention.
