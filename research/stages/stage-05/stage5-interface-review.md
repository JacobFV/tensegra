...
## Separately labeled selector-read supplement

Independently reviewed model change `4b505f3` and runner/configuration change `4ca8fb9`. The learned selector controls add a probability-weighted node-memory read and an operation-distribution embedding to their neural recurrent update. They do not execute exact pointers or arithmetic. The oracle-selector control explicitly receives gold semantic equivalence distributions through a separate override argument, so it is privileged and must not be described as learned lowering. It retains ambiguity between same-named runtime slots/scopes; it does not receive gold destination values or future execution state.

All original ten variant defaults remain frozen. The three supplemental variants share the same parameter shapes, seeded initialization, training-data schedule, auxiliary binding targets, and result/output targets as their corresponding neural controls. They add an architectural retrieval prior and a task-gradient path into the lowerer. This is a useful stronger baseline, not a declaration that all plausible graph/message-passing interfaces have been exhausted. Oracle semantic reads are averages over equivalent selectors, while exact execution resolves those selectors in current runtime context; report this difference.
