...
Status: archived reconstruction, frozen-checkpoint probes and all-seed readout repair complete; final independent audit pending. No return interface gate passes, no repair selected, no downstream use authorized. Workspace standard remains 1024.
...
All three new width1024 backbone runs and both paired readout refits completed without recipe changes. **All nine seed/readout retention gates fail.** The five non-value predictions are not regenerated by refitting; all five non-value output arrays are exactly identical across the three readout arms on every paired example.
...
The original scalar-grid fixture is preserved under `return-grid`. A final schema audit found that its unary-to-binary conversion could reuse argument0 as the replacement argument1 while operand witnesses differed. Version`fbff194` selects a distinct public identity, with a direct contract test; corrected results are under `return-grid-v2`. All18 raw paths and all six selected facets remain528/528; mixed/concat linear-read counts become293–364/528. This correction affects only the isolated grid fixture, not historical data, frozen boundary probes, or the prespecified repair main. Five focused mechanical tests pass.

At16 updates with eight distractors, the three-seed descriptive conditional count `value correct | every non-value field correct` is1,091/1,445 for the original head,1,293/1,445 for ridge and1,236/1,445 for CE. Scalar errors therefore remain even within otherwise correctly reconstructed events. These are directly counted intersections, not products of marginal accuracies. Validation readout selection pools four short delays and two distractor conditions of the same512 events; those4,096 feature rows are not4,096 independent semantic examples.
