...
Profile at `37a24d3`: 64 events in each of three fresh partitions, all seven recurrent delays, one source-delay fit, took **1.77 seconds** after model construction on the configured GB10. Four mechanical tests passed, including original-forward feature and non-value prediction parity. The full three-checkpoint matrix is estimated at **2–3 minutes**, with a **5-minute hard stopping limit**, including feature caching and all fits. It does not approach the 35-minute track ceiling. There are no optimizer updates: report the 36 closed-form solves per backbone, 122,880 supervised fit-row uses, and only 2,048 distinct training events separately. Lossless float32 cache bytes and fitted coefficients stay on the linked machine with hashes; compact raw predictions and selection records are committed.
