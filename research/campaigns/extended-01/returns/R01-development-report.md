...
Independent raw-count/selection audit passed all 90 cells for both profile and development, reconstructed the chosen heads, and confirmed unchanged non-value predictions. Independent CPU replay and provenance checks passed, including train-only normalization, event disjointness and coefficient/cache hashes. No confidence-calibration claim is made from ridge class scores. No returned-fact use or composition experiment has run yet.
