...
Before either fork,3,600updates were replayed from the original Stage11scalar head. Weights, normalization, full loss sequence and sampled-row membership were checked for exact equality before proceeding. During endpoint export after the forks, every archivedR10reference prediction, target and event hash was also checked and matched exactly. The old optimizer state was reconstructed by deterministic replay; it was not available as a historical checkpoint. Historical index-order hashes were not recorded, so historical order is justified by the frozen sampler recipe plus these replay equalities, not an unavailable old hash.
...
Full outer process occupancy57.58seconds (GNU time,.01sresolution); load/preparation11.585s, exact replay3.191s, fork fitting5.589s, endpoint evaluation10.950s, remaining child work/export24.446s. Peak CUDA allocation4,835,127,808bytes; peak process RSS8,422,756,352bytes. Lossless remote logits and checkpoints plus all108compact raw cells/source/config/provenance are retained. Independent raw/cache/state/gate audits pass; the fixed promotion failure and lower-LR relative effect are independently confirmed. This experiment concerns supporting scalar interfaces, not programmable attention.
