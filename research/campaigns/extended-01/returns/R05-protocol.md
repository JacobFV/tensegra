...
Before any R05 outcomes, the exact public generator implies a query-only Bayes ceiling of **79.4123%**: 20% Boolean, 40% uniform half-unit and 40% ties-to-even rounded half-unit values, with independent uniform thresholds and polarity. See `R05-query-prior.json` and its stdlib reconstruction tool. Above-chance query-only behavior is therefore expected, not evidence of leakage. The registered 20-point drop criterion is stringent: a perfect predictor can exceed the no-return Bayes ceiling by only 20.5877 points. Retain that criterion and disclose its failure if observed; separately report original/supplied-fact intervention sensitivity rather than retroactively relaxing it.
