...
For wrong/swap, retain both original and supplied-fact labels. Report all-example and answer-changing-subset accuracies, with actual subset support. Intended-use evidence requires at least a 15-point drop in original-answer accuracy when the event is absent, at least 80% of the matched exact-value-minus-query-only clean accuracy improvement, and at least 90% agreement with the supplied fact on answer-changing wrong/swap subsets (minimum 256 such examples per cell). These are prospective development checks, not implied by a clean accuracy pass. Failures select a diagnostic branch instead of changing thresholds.
...
Before any R05 outcomes, the exact public generator implies a query-only Bayes ceiling of **79.4123%**: 20% Boolean, 40% uniform half-unit and 40% ties-to-even rounded half-unit values, with independent uniform thresholds and polarity. See `R05-query-prior.json` and its stdlib reconstruction tool. Above-chance query-only behavior is therefore expected, not evidence of leakage. The initial 20-point draft threshold left only 0.5877 points below the perfect-to-Bayes improvement, inconsistent with the >95% development acquisition threshold. Before any R05 training or outcomes, the coordinator authorized a versioned contract repair: at least 15 points of original-answer drop under event removal, plus at least 80% of the matched exact-value-versus-query-only clean improvement. The >=90% supplied-fact changed-subset criterion and >=98% three-population composition threshold remain unchanged. Report every absolute effect and paired uncertainty. This amendment is dated 2026-09-23 and justified solely by the analytical observation model; prior drafts remain in Git history.

The normalized clean improvement is `(accuracy_learned - accuracy_query_only) / (accuracy_oracle - accuracy_query_only)` on the same events and delay. If the denominator is nonpositive, the criterion is undefined and cannot pass. No clipping hides negative or greater-than-one ratios.
