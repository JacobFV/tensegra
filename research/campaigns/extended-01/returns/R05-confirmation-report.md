...
External process occupancy was352.54seconds, including all capture, fitting and export. All513raw cells, cached accessor/consumer inputs, scalar scores, logits, coefficients and source/config/checkpoint hashes are retained. Independent audit9cc33af verified all513raw and513cache/logit cells, source/weight hashes,23disjoint populations, paired causal prefixes and per-run uncertainty. The scoped validation and causal checks pass allthree runs. Any further composition decision belongs to the coordinator and must preserve this narrow contract; historical failed scalar/interface gates remain unchanged.
