...
import argparse,base64,collections,gzip,hashlib,json,math,time
...
def target_edges(t):
 e=t['edges'];require(e['bitorder']=='little','edge bit order');shape=e['shape'];require(len(shape)==3 and all(type(x)is int and x>0 for x in shape),'positive edge dimensions');n,m,r=shape
 require(n==m and len(t['slots'])==n and all(len(row)==m and all(type(v)is int for v in row) for row in t['slots']),'square target/slot dimensions')
 buf=base64.b64decode(e['packed_b64'],validate=True);require(len(buf)==(n*m*r+7)//8,'exact packed edge length');out=[]
...
def row_guard(row):
 require(type(row['valid']) is bool and type(row['complete']) is bool,'strict boolean output flags')
 checked=canonical_records(row['records'],[]);require(len(checked)<=160,'generated record capacity')
 if row['valid']:
  require(bool(checked) and checked[-1][0]==3,'valid sequence must terminate at EOS');seen_edge=False
  for rec in checked:
   if rec[0]==2:seen_edge=True
   require(not(seen_edge and rec[0]==1),'valid NODE after EDGE')
 require(set(row['exact_components'])==set(COMPONENTS) and all(type(v)is bool for v in row['exact_components'].values()),'strict component flags')
 if row['valid']:
  require(row['reason'] is None and row['prediction'] is not None and row['metrics'] is not None,'valid output linkage')
  require(row['complete']==all(row['exact_components'].values())==bool(row['metrics']['semantic_equivalence']),'exact output linkage')
 else:require(not row['complete'] and isinstance(row['reason'],str) and bool(row['reason']) and row['prediction'] is None and row['metrics'] is None and not any(row['exact_components'].values()),'invalid outputs receive no credit')
 t=row['target'];require(t['edges']['shape']==[128,128,13],'historical target tensor dimensions')
 for k in ('presence','kind','value','copy'):require(len(t[k])==128,'target node dimensions')
 require(all(type(v)is bool for v in t['presence']),'boolean target presence')
 for k in ('kind','value','copy'):require(all(type(v)is int for v in t[k]),'integer target fields')
 require(t['presence']==[True]*sum(t['presence'])+[False]*(128-sum(t['presence'])),'canonical target prefix')
 target_edges(t)
def entry_guard(entry,d):
 rows=d['rows'];require(entry['examples']==len(rows),'entry example count');require(entry['teacher_forced']==d['teacher_forced'] and entry['teacher_forced_loss']==d['teacher_forced_loss'],'teacher-forced summary linkage')
 for field in d['teacher_forced'].values():
  require(type(field['count'])is int and field['count']>0 and type(field['correct'])is int and 0<=field['correct']<=field['count'],'teacher-forced support')
  require(math.isclose(field['mean_loss'],field['loss_sum']/field['count']) and math.isclose(field['accuracy'],field['correct']/field['count']),'teacher-forced arithmetic')
 require(set(d['teacher_forced'])=={'type','kind','value','copy','source','target','role','slot'} and math.isclose(d['teacher_forced_loss'],sum(x['mean_loss'] for x in d['teacher_forced'].values())/8),'field loss recipe')
 require(sum(r['complete'] for r in rows)==entry['complete'] and sum(r['valid'] for r in rows)==entry['valid'],'manifest score binding')
 require(dict(collections.Counter(r['reason'] for r in rows if not r['valid']))==entry['invalid_reasons'],'invalid reason summary')
 cells={c:dict(examples=sum(r['cell']==c for r in rows),complete=sum(r['complete'] for r in rows if r['cell']==c)) for c in {r['cell'] for r in rows}}
 require(cells==entry['cells'],'entry cell summary')
def public_guard(row,public):
 require(row['seed']==public['seed'] and row['graph_sha256']==public['graph_sha256'] and row['cell']==f"{public['arity']}x{public['facts']}",'DEV identity differs from public cache')
...
def describe(row,tokens,templates):
 ...
 nonleading_nodes=sum(x[0]==1 for x in records[len(pn):]);pe=[tuple(x[1:]) for x in records if x[0]==2];ge=target_edges(t);missing=sorted(set(ge)-set(pe));extra=sorted(set(pe)-set(ge));slot_only=sum(any(x[:3]==y[:3] and x[3]!=y[3] for y in pe) for x in missing)
 ...
 elif nonleading_nodes:category='node:nonleading_node'
 ...
 return dict(semantic_sha256=row['semantic_sha256'],cell=row['cell'],valid=row['valid'],reason=row['reason'],complete=row['complete'],category=category,generated_leading_nodes=len(pn),nonleading_node_records=nonleading_nodes,gold_nodes=n,first_node_index=f,first_node_fields=fields,first_node_predicted=None if f is None or f>=len(pn) else pn[f],first_node_gold=None if f is None or f>=len(gn) else gn[f],kind_sequence_correct=kindseq==tuple(x[0] for x in gn),entity_removed_layout_correct=skeleton==goldsk,matching_layouts=[k for k,v in templates.items() if skeleton==v],eos_present=any(x[0]==3 for x in records),missing_edge_count=len(missing),extra_edge_count=len(extra),slot_only_missing_count=slot_only,first_missing_edge=missing[0] if missing else None,first_extra_edge=extra[0] if extra else None,first_positional_edge_difference=first_difference(pe,ge),duplicate_edge_count=len(pe)-len(set(pe)),leading_nodes_correct=f is None,all_nodes_correct=f is None and nonleading_nodes==0,edges_correct_after_nodes=f is None and nonleading_nodes==0 and not missing and not extra,raw_record_count=len(records))
def cell_summary(rows):
 return dict(examples=len(rows),counts={k:sum(bool(r[k]) for r in rows) for k in ('valid','complete','leading_nodes_correct','all_nodes_correct','kind_sequence_correct','entity_removed_layout_correct','eos_present','edges_correct_after_nodes')},categories=dict(collections.Counter(r['category'] for r in rows)),invalid_reasons=dict(collections.Counter(r['reason'] for r in rows if not r['valid'])),first_node_fields=dict(collections.Counter(','.join(r['first_node_fields']) or 'none' for r in rows)),first_node_indices=dict(collections.Counter(str(r['first_node_index']) for r in rows)),matching_layouts=dict(collections.Counter(','.join(r['matching_layouts']) or 'other' for r in rows)),generated_leading_node_counts=dict(collections.Counter(str(r['generated_leading_nodes']) for r in rows)),edge_error_counts_after_correct_nodes=dict(missing=sum(r['missing_edge_count'] for r in rows if r['all_nodes_correct']),extra=sum(r['extra_edge_count'] for r in rows if r['all_nodes_correct']),slot_only_missing=sum(r['slot_only_missing_count'] for r in rows if r['all_nodes_correct'])))
   ...
   for pop in ('train','development'):
    ...
    if pop=='development':
     for key,row in ix.items():
      require(key in public,'unknown public DEV identity');public_guard(row,public[key])
