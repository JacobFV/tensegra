...
def main():
 ...
 for arity in (2,3,4):
  ...
  for wanted in (3,4):
   ...
   for attempt in range(256):
    seed=921000000+arity*100000+wanted*1000+attempt;difficulty={2:0.,3:1/3,4:.5}[arity];e=build_tcn_example('unification',seed,difficulty=difficulty,languages=('english',));g=e.privileged.graph;nodes=list(g.nodes);ids={n.id:i for i,n in enumerate(nodes)};edges=[(ids[x.source],ids[x.target],x.role,x.slot) for x in g.edges]
 ...
 (ROOT/'research/results/campaign-01/semantics/s21-generator-feasibility.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
