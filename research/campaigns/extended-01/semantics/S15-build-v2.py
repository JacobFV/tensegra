...
import random
out=base/'s15-shape-cache-v2';assert not out.exists();tick=time.monotonic();seen=set(excluded);texts={};records={};stats={}
...
# Retain v1 confirmation and DEV; reserve additional fresh DEV before TRAIN reselection.
for split in ('development','confirmation'):
 rows=[json.loads(line) for line in gzip.open(base/'s15-shape-cache'/(split+'.jsonl.gz'),'rt')]
 for row in rows:
  assert row['alpha_sha256'] not in seen
  accept(row)
 records[split]=rows
for arity in (3,4):
 cells=collections.Counter()
 for attempt in range(10000):
  row=record(arity,918000001+(arity-2)*100000+attempt)
  if row is None or cells[row['facts']]>=256:continue
  accept(row);records['development'].append(row);cells[row['facts']]+=1
  if cells[3]==cells[4]==256:break
 assert cells[3]==cells[4]==256,(arity,dict(cells))
 stats[f'development-extension-{arity}']=dict(attempts=attempt+1,selected=dict(cells))
assert len(records['development'])==len(records['confirmation'])==2048
