...
CPU-only residual localization finds calibrated DEV errors dominated by `refers_to` (2,255false positives/3,613false negatives), followed by `argument` (1,098/631). The largest reference miss cell contains3,432legitimate ident→entity pairs, so type masking alone cannot remove the error. Privileged replacement of only `refers_to` produces27/512complete DEVgraphs under the original calibrated policy; replacing both `refers_to` and `argument` produces156/512. Every other single-relation replacement gives0. Under raw decoding, goldreferences alone still gives0 and references+arguments gives37/512. These are component-replacement ceilings, not deployable inference or proof that the replaced relation has been learned. They motivate the separately registered S05 public predicted-identity contract diagnostic.
