...
def main():
 ...
 for a,run in zip(manifest['artifacts'],config['runs'],strict=True):
  ...
  new=load(newpath)['rows'];old={r['semantic_sha256']:r for r in load(oldpath)['rows']};assert len(new)==2048 and len({r['semantic_sha256'] for r in new})==2048 and set(old)==set(cache)
