...
with tempfile.TemporaryDirectory() as tmp:
 ...
 (src/'actor.py').write_text('fixture');timeout_config=root/'timeout-config';m.freeze(prepared,timeout_config,src,.2)
 sleeper=root/'sleeper';sleeper.write_text('#!/usr/bin/env python3\nimport time\ntime.sleep(2)\n');sleeper.chmod(0o755);timeout_prefix=root/'timeout'
 result=subprocess.run([sys.executable,str(src/'campaign_semantics_recalibrate_launch.py'),str(timeout_config),'--cap','.2','--python',str(sleeper),'--prefix',str(timeout_prefix)],env={**os.environ,'PATH':str(binary)+os.pathsep+os.environ['PATH']},capture_output=True,text=True)
 receipt=json.loads(Path(str(timeout_prefix)+'.occupancy.json').read_text());assert result.returncode==124 and receipt['timed_out'] and receipt['exit_code']==124 and receipt['preflight_seconds']<.2 and .2<=receipt['process_occupancy_seconds']<1.5
print(json.dumps(dict(selection_replay=True,prepared_recipe_rejection=True,immutable_freeze=True,cap_source_selection_checkpoint_cache_guards=True,durable_failure_receipt=True,whole_clock_timeout_receipt=True,no_model_import_or_inference=True,no_real_gpu_query=True)))
