...
Development seed1901; batch8;4,096updates/32,768presentations/eight visits. Reset the same dedicated torch.Generator seed15115 and epoch permutations as S18, independent of model seed, labels and evaluation; verify the construction-stream hash. AdamW LR3e-4 with128-update linear warmup and cosine decay to3e-5, betas(.9,.999), weight decay.01, gradient norm cap1; BF16 forward with floating-point cross-entropy. Average losses by applicable field and record, log every field independently, and disclose exact loss aggregation before freeze. No calibration or best-checkpoint selection.
...
Graph equivalence preserves the historical edge-set contract: distinct EDGE records may be emitted in any order. TRAIN serialization stays sorted; canonical edge-list order is a separate diagnostic, not a semantic failure. Node emission indices and ordered slot labels remain exact. This supplies no gold reordering or graph repair. Exact duplicates, dangling references and incompatible slots remain invalid.

Applicable value/copy output heads predict only valid field-domain classes: finite value vocabulary and public token positions, respectively. No never-supervised missing-value output class is added. Unused input payload fields remain-1; unordered edge slot remains a real output class. Excluding syntactically impossible missing required fields does not choose the correct value or pointer.

Public-length diagnostic before S19 outcomes: all inspected DEV3×4 and4×3 inputs contain54tokens, whereas3×3 has46 and4×4 has64. Thus the three trained motifs are distinguishable by length alone, but the heldout combination shares length with a different trained motif. Known-motif acquisition alone cannot establish compositional parsing; the heldout test demands using additional public content. This is a measured population property, not proof that a model uses a length shortcut.
