...
Profile completed32 updates on256 distinct examples in2.33538 optimizer seconds; runner-body12.04809 seconds (excluding Python imports) including64 development and64 TRAIN forwards across two checkpoints, exact calibration-array exports and checkpoint serialization. Parameter count57,853,781; peakCUDA allocation1,225,688,064 bytes; processmaxRSS2,909,528 KiB. Extrapolated optimizer time per8192-update arm is598 seconds; allow roughly120 seconds for target-cache/CPU preparation, four full512-development evaluations, calibration and export. Expected12 minutes perarm; requested hard15-minute perarm cap (45-minute total ladder ceiling). Each arm still requires its own coordinator queue release. Budget is based on runtime, not profile accuracy.

External profile UTC receipts span20:05:00–20:05:13Z; the campaign conservatively charges14 seconds for full process occupancy. The12.04809-second inner measurement is retained separately and must not be labeled complete occupancy. Main runs use an external monotonic subprocess wrapper that includes interpreter startup, all evaluation/export, and process exit.
