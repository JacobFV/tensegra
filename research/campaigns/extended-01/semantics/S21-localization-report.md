...
Both arms fail first at the identical zero-based canonical node indices:8 in12 examples,9 in157, and10 in343. Every first discrepancy changes kind/value/copy together: predicted `ident` versus required `pred(parent)`. The identifier appears where the arity-three fact has ended in the canonical target serialization. This is consistent with continuing an argument list, but the first-node measurement alone does not establish the generated edge assignment. Neither arm produces a completely correct node-kind sequence, correct entire node records, or even the correct entity-removed occurrence-tree layout for this cell.
...
These are canonical-position diagnostics. These canonical-position failures do not by themselves test unrestricted graph isomorphism or rule out every node permutation. Removing entities is used only to describe layout, never to relax exact recovery.
...
Independent review reconstructed all7,168 event descriptors, cell summaries and paired tables successfully(16.347s CPU). Report wording was narrowed afterward: canonical-position diagnostics alone neither establish a particular argument-edge assignment nor rule out every node permutation. No numerical output or source analysis changed.
