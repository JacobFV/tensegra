...
Unchanged joint mean-head argmax path accuracy is99.02%; individual head-node-step argmax correctness averaged over all nodes is73.87% at the record read and73.47% at the destination read. These have different supports: queried complete paths versus all-node/head/step averages. Their numerical gap alone is not a direct estimate of hidden routing error along queried paths. Only7.67% of all joint node-steps have all eight heads select the same record destination;7.53% have all heads select the same destination-read node. Per-head record correctness ranges65.05–79.17%, so all-node disagreement is not confined to one anomalous head. Correct mean-head paths do not establish every head executing that pointer, but these metrics do not prove that heads were intended as exact pointers rather than a distributed computation. Query-trajectory head statistics were not archived by A12 and require separate prospective instrumentation.
