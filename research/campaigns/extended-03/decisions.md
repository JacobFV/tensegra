...
- 2026-09-26T14:44Z: **Part D done** (exit 0; 3,251 core-s, 1,137 s wall). Report: research/results/campaign-03/p2a-partd/partd.md (64 probe worlds; greedy + 2 samples per world). Descriptive readings, no causal claim:
  1. **The P1 "collapse" is a greedy-deployment collapse.** Sampled success stays .80–.93 through attempt 29 in all three lineages (r2: bootstrap .875 → a29 .922), while greedy success falls to .09 (r0) and .00 (r2) from attempt 26. The divergence (≥ .10) appears first at attempt 26 in both collapsed lineages and never in r1. At the bootstrap, greedy (.95) already beats sampled (.85–.88).
  2. **KL to the bootstrap rises sharply only on the checkpoint's own states and only at the collapse** (r2: .41 → .69 → .96, argmax disagreement .21 → .75). On the bootstrap's states it stays about .22–.53. KL to the previous tranche stays ≈ .01–.03 throughout; each tranche step is small.
  3. **The critic does not favour loop actions:** the standardized advantage of loop actions is below that of productive actions in 7 of 8 pre-collapse checkpoints (r0 and r2).
  4. **The critic dominates the shared encoder.** The bootstrap critic starts miscalibrated (MSE .51–.74, bias −.45 to −.72). The critic's gradient norm on shared parameters is 30–80× the actor's at the bootstrap and remains larger at nearly every checkpoint. The clip fraction is 1.00 everywhere, so gradient clipping is always active and the direction of each shared step is mostly the critic's. cos(actor, critic) ≈ 0.
  - Candidate explanation (**not established**): RL mostly reshapes the sampled policy, where the objective lives, while the argmax path on the policy's own states drifts into ties and loops. Critic-dominated shared updates are a candidate driver of that representational drift.
  - Part C proceeds unchanged. Its screening already reports greedy and sampled modes, and S1 (greedy) is read alongside the sampled readings.
- 2026-09-26T15:20Z: **Profiling** (C0, first 24 tranches; phase CPU from state.json `training_timing.phase_timing`; `rollout_total` is an umbrella over the phases below and is excluded).
