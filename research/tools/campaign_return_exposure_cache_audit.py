...
packed=Path(cfg['feature_path']).read_bytes();assert sha(packed)==cfg['feature_sha256'];raw=gzip.decompress(packed);assert sha(raw)==ref['features']['uncompressed_sha256'];f=torch.load(io.BytesIO(raw),map_location='cpu',weights_only=True);saved=cache('logits')
...
heads={}
for fit in m['endpoints']:
 n=m['fit_unique_events'];step=fit['step'];path=a.root/f'ce-{step}.pt';assert sha(path.read_bytes())==fit['checkpoint_sha256'];head=torch.load(path,map_location='cpu',weights_only=True)
 ...
 heads[f'ce_{step}']=head
 if step==900:
  reference=torch.load(cfg['reference_path'],map_location='cpu',weights_only=True);assert sha(Path(cfg['reference_path']).read_bytes())==cfg['reference_sha256']
  assert torch.equal(head['mean'],reference['mean'])and torch.equal(head['scale'],reference['scale'])
  for key,value in head['state'].items():assert torch.equal(value,reference['state'][key])
sets=[set(f[s+'/2']['event_row_hashes'])for s in ('train','calibration','validation')];assert all(not(l&r)for i,l in enumerate(sets)for r in sets[i+1:]);checks=[]
