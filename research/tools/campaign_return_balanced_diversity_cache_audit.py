...
heads={}
for fit in m['fits']:
 n=fit['events'];path=a.root/f'balanced_{n}.pt';assert sha(path.read_bytes())==fit['checkpoint_sha256'];head=torch.load(path,map_location='cpu',weights_only=True)
 ...
 assert f['train/2']['event_row_hashes'][:n]==fit['training_event_hashes'];heads[f'balanced_{n}']=head
sets=[set(f[s+'/2']['event_row_hashes'])for s in cfg['data']];
sets.append(set(f['train/2']['event_row_hashes']))
for key in cfg['grids']:sets.append(set(f[key+'/8']['event_row_hashes']))
