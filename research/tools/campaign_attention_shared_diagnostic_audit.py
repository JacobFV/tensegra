"""Independent A13 task/path and archived diagnostic contract reconstruction."""
import argparse,hashlib,json,subprocess,time
from pathlib import Path
import numpy as np
p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('--source-ref',required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args();tick=time.monotonic();cfg=json.loads((a.root/'config.json').read_text());m=json.loads((a.root/'manifest.json').read_text());policies=['unchanged','shared_soft','shared_hard','oracle_common'];assert m['policies']==policies and m['optimizer_updates']==0;assert m['initial_tensor_sha256']==m['final_tensor_sha256']==cfg['tensor_sha256'];assert m['checkpoint_sha256']==cfg['checkpoint_sha256']
...
for policy in policies:
 ...
 for ci,row in enumerate(report['rows']):
  ...
  if policy!='unchanged':assert np.all(dg('used_head_address_discrepancy')==0)
  if policy in ('shared_hard','oracle_common'):assert np.all(dg('payload_mse_to_destination_argmax')==0);assert np.all(dg('payload_mse_correct_destination_sum')==0)
