...
import numpy as np
p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('--output',type=Path,required=True);p.add_argument('--replicates',type=int,default=4000);a=p.parse_args();started=time.monotonic();rng=np.random.default_rng(912003);results=[]
def wilson(k,n):
...
for seed in (10,11,12):
 ...
 for split in ('validation','test'):
  ...
  for head in ('unchanged','ridge','ce'):
  ...
  for reference in ('unchanged','ridge'):
   delta=mat['ce']-mat[reference];rep=weights@delta/n;intervals=np.quantile(rep,[.025,.975],axis=0)
   comparisons[reference]=[dict(delay=d,distractors=k,ce_minus_reference=float(delta[:,i].mean()),percentile95=intervals[:,i].tolist())for i,(d,k)in enumerate(conditions)]
...
out=dict(cpu_audit_wall_seconds=time.monotonic()-started,replicates=a.replicates,bootstrap_seed=912003,results=results,scope='Each seed/split resamples the same event indices jointly across arms/delays/distractors. Marginal Wilson95 and descriptive percentile intervals, not simultaneous guarantees. Different seeds remain separate. Empirical gate decisions are unchanged.');a.output.write_text(json.dumps(out,indent=2)+'\n');print(dict(cpu_seconds=out['cpu_audit_wall_seconds'],seed_split_populations=len(results)))
