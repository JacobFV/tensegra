...
for row in rows:
 ...
 if not f.is_file():bad.append(dict(path=path.decode(),reason='missing'));continue
 data=f.read_bytes();got=hashlib.sha1(b'blob '+str(len(data)).encode()+b'\0'+data).hexdigest()
