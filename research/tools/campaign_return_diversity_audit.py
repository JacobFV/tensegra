...
def audit(root,ref):
 ...
 for r in rows:
  ...
  assert r['counts']['value']['total']==(52*c['balanced_grid']['per_cell']if r['split']=='balanced'else c['data'][r['split']]['size'])
  if r['split']=='balanced':
   strata=Counter(zip(r['targets']['type'],r['targets']['value']));assert strata==Counter({(typ,label):c['balanced_grid']['per_cell']for typ,labels in ((0,range(0,33,2)),(1,range(33)),(2,(16,18)))for label in labels})
 ...
 expected={(s,k,d,a)for s in c['data']for k in ([2]if s=='train'else c['eval_distractors'])for d in (c.get('test_delays',c['delays'])if s=='test'else c['delays'])for a in arms};expected|=({('balanced',8,d,a)for d in c['balanced_grid']['delays']for a in arms}if 'balanced_grid'in c else set());assert set(idx)==expected
 for (s,k,d,a),r in idx.items():
  base=idx[s,8 if s=='balanced'else 2,0,'unchanged'];assert r['targets']==base['targets']and r['event_sha256']==base['event_sha256']
