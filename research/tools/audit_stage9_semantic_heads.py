...
def audit(paths):
 ...
 for path in paths:
  ...
  for run in data['runs']:
   ...
   last=run['curve'][-1];counts.append(dict(file=path.name,seed=run['seed'],head=run['head'],objective=run['objective'],final_exact=sum(r['exact_graph']for r in last['rows']),graphs=len(last['rows']),updates=last['update']))
