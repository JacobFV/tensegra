"""Independent CPU C04 compact profile counts; no neural inference."""
import json,hashlib,time,argparse
from pathlib import Path
import torch
p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('--manifest',type=Path,required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args();t=time.monotonic();torch.set_num_threads(2);sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest();summaries={};files=0;cells=0
for phase,entries in json.loads(a.manifest.read_text()).items():
 r=a.root/f'c04-profile-{phase}';summaries[phase]=json.loads((r/'summary.json').read_text())
 for item in entries:
...
r=a.root/'c04-profile-hybrid';m=summaries['hybrid'];assert m['frozen_state']['before']==m['frozen_state']['after'];load=lambda p:torch.load(p,map_location='cpu',weights_only=False)
def check(c,name):
 global cells
...
for arm in ('n1_static','n1_roles','n2_rekey'):
 r=a.root/f'c04-profile-{arm}';m=summaries[arm];assert m['primary_endpoint_step']==16
...
for c in summaries['timing']['cells']:
 for rec in c['passes']:
  x=load(a.root/'c04-profile-timing'/f"{c['path']}-b{c['batch']}-d{c['delay']}-r{rec['repeat']}.pt");assert count(x['predictions'],x['targets'])==dict(correct=rec['correct'],total=rec['attempted']);assert int((x['predictions']<0).sum())==rec['refused'];cells+=1
...
out=dict(raw_files_verified=files,primary_cells_reconstructed=cells,paired_sample_stream=True,n1_initial_states_equal=True,cpu_audit_wall_seconds=time.monotonic()-t,scope='Mechanical16event/16update profile only: raw primary counts, calibration selection, all artifact hashes and timing outcomes. No acquisition/gate or throughput guarantee; subgroup/arithmetic/full causal gate reconstruction reserved for main audit.')
