...
import numpy as np
p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('--seed',type=int,required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args();start=time.monotonic();arms=['soft4','soft8','context','message','hard'];allrows={};initial=[];checks=[];permutations=[]
for arm in arms:
 ...
 for i,r in enumerate(rows):
  ...
  if c.get('node_permutation'):
   diff={field:int((raw[f'c{i}_{field}']!=raw[f'c{base}_{field}']).sum())for field in ('pred','route','task')}
   permutations.append(dict(arm=arm,changed=diff,payload_positions=int(raw[f'c{i}_pred'].size)))
   assert diff['route']==diff['task']==0
...
out=dict(seed=a.seed,cpu_audit_wall_seconds=time.monotonic()-start,permutation_checks=permutations,paired_final_cells=len(checks),initial_shared_tensor_sha256=initial[0],checks=checks,scope='Final clean targets and public graph identifiers paired across arms/interventions; coordinate-restored route/task invariance checked; intermediate payload discrepancies explicitly counted. Same1024 event draws are reused across arms; cross-seed identity requires the completed matrix audit.');a.output.write_text(json.dumps(out,indent=2)+'\n');print(dict(seed=a.seed,cells=len(checks),seconds=out['cpu_audit_wall_seconds']))
