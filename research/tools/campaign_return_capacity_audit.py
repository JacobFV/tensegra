"""Independent R07 counts and matched linear endpoint reconstruction."""
...
for f,h in m['source'].items():assert hashlib.sha256(subprocess.check_output(['git','show','844497b:src/topoformer/'+f])).hexdigest()==h
for r in rows:
...
assert len(idx)==84
for arm in ('linear','residual_mlp'):
 summary[arm]={}
 for split in ('train_balanced','calibration','validation','calibration_grid','validation_grid'):
  ...
  summary[arm][split]=s
assert m['fits'][0]['visited_row_bits_little_endian']==m['fits'][1]['visited_row_bits_little_endian']
