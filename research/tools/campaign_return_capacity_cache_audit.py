"""Independent CPU R07 nonlinear readout replay from immutable features."""
 ...
 for key,b in cache.items():
  for delay,x in b['features'].items():
   for arm,h in heads.items():
    ...
    got=linear(z,s,'') if arm=='linear' else linear(z,s,'linear.')+linear(torch.nn.functional.gelu(linear(z,s,'hidden.')),s,'output.')
