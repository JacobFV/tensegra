...
p=Path('research/results/stage7-ab/a2-main');m=json.loads((p/'manifest.json').read_text());summary=json.loads((p/'summary.json').read_text());assert hashlib.sha256(json.dumps(m['config'],sort_keys=True).encode()).hexdigest()==m['config_hash']
