...
x=json.loads(gzip.decompress(Path('.development/stage7-halt-main-raw-metrics.json.gz').read_bytes()));c=json.loads(x['config.json']);m=json.loads(x['manifest.json']);g=json.loads(x['gate.json']);assert hashlib.sha256(x['config.json'].encode()).hexdigest()==m['config_sha256']
