...
Style breakdowns now preserve numeric-report, comparison and sign task/lifting counts with their defined-query denominators. Undefined queries have no task/execution/lifting outcome even if gold null lowering is correct. Seven standalone regression tests pass, including this boundary.

Post-freeze analysis correction: confidence result-risk and result-accuracy are undefined on ambiguous/invalid batches with no defined target. Invocation, deferral and rejection coverage remain observable and receive separate plots. Mixed defined/undefined batches cannot support answer-risk without a cross-count of answered defined examples; that risk also remains null. FP/FN lowering measures retain their declared raw-action populations. The CLI records its own source-file SHA separately from immutable training provenance. This changes artifact interpretation only; no training source or metrics are altered. Eight standalone tests pass.
