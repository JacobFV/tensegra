...
reused for two microsteps. Tests explicitly use width16; they are contract tests,
...
reserved64-graph prefix across N; train sets are nested disjoint prefixes.

The first1000 presentations supervise node presence/types, then values and visible
identity copying enter, and sampled typed/ordered edges enter at2000. Every loss
...
Training scores all gold-positive edges plus up to128 sampled negative node
...
are computed for evaluation. A width128 edge compatibility head is not a narrow
workspace: all four recurrent phases, node states and attention operate at1024.
...
The1k/10k comparison uses one common10k training-only vocabulary pool, excluding
the reserved heldout prefix. This keeps categorical value-head capacity equal.
Larger100k configurations use a100k pool; to compare their capacity exactly with
...
Inputs never include gold graph size, spans, nodes or edges. Decoder capacity128,
...
This differs from Stage7 in width, actor simplification, curriculum and sparse
loss sampling. It cannot isolate a single cause of improvement over Stage7.
Within Stage8 the N×E axes hold those choices fixed. No composition, annealing,
