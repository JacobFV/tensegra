...
def losses(out,gold):
    ...
    result=dict(presence=F.binary_cross_entropy_with_logits(out['presence'],present.float()),
        kind=F.cross_entropy(out['kind'][present],gold['kind'][present]),edges=.5*(raw[positive].mean()+raw[~positive].mean()),
        slots=F.cross_entropy(out['slots'].flatten(0,1), (gold['slots']+1).flatten()))
...
def metrics(pred,gold):
    ...
    semantic=bool(not gold['value'].eq(0).any() and p.eq(mask).all() and correct['kind'][mask].all() and edges.eq(gold['edges']).all()
        ...
        and pred['slots'][gold['slots'].ge(0)].eq(gold['slots'][gold['slots'].ge(0)]).all())
    ...
    with torch.no_grad():
        for e in examples:
            ...
            if raw_path is not None:
                ...
                with gzip.open(raw_path,'at') as stream:
...
def run(config):
    ...
    train_count=config['train_count']; heldout=[corpus[i] for i in range(train_count,len(corpus))]
    vocab=value_vocabulary(corpus[i] for i in range(train_count))
    capacity=config['node_capacity']; source={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name('tcn_data.py'),Path(__file__).with_name('semantic_graph.py'),Path(__file__).with_name('thinking.py')]}
    ...
    for i in range(train_count):
        e=corpus[i]; public,g=surface_input(e,'english'); target=targets(g,public,capacity,vocab)
    ...
    def record(model,seed,arm,step,start,seen,tokcount):
        ...
        row['fixed_training_set']=evaluate(model,[corpus[i] for i in range(min(train_count,config.get('acquisition_count',8)))],'english',vocab,capacity)
        records.append(row)
        ...
        for arm in ('semantic','no_input'):
            ...
            for step in range(config['updates']+1):
                ...
                for j in range(config['batch_size']):
                    index=(step*config['batch_size']+j)%train_count; e=corpus[index]
                    language=('english','spanish')[(step//max(1,math.ceil(train_count/config['batch_size'])))%2]
                    public,g=surface_input(e,language)
                    seen.add(index); tokcount+=len(tokens(public)); component=losses(model(public),targets(g,public,capacity,vocab,language=language)); parts.append(sum(component.values()))
                loss=torch.stack(parts).mean()
                if not torch.isfinite(loss): raise FloatingPointError('nonfinite loss')
                loss.backward(); nn.utils.clip_grad_norm_(model.parameters(),1.); optimizer.step()
                with (outdir/'losses.jsonl').open('a') as stream: stream.write(json.dumps(dict(seed=seed,arm=arm,step=step+1,loss=float(loss.detach()),parts={k:float(v.detach()) for k,v in component.items()}))+'\n')
